...
void generate_boundary_instance_witness(Bullet_PP &pp, 
                                        ...
                                        bool BOUNDARY_FLAG)
{
    if(BOUNDARY_FLAG == 0) cout << "generate left boundary" << endl;  
    BIGNUM *exp = BN_new(); 
    BN_set_word(exp, pp.RANGE_LEN);
    ...
    {
        BN_random(witness.r[i]);
        if (BOUNDARY_FLAG == 0) BN_zero(witness.v[i]);
        else BN_mod_exp(witness.v[i], BN_2, exp, order, bn_ctx); 
        BN_print_dec(witness.v[i], "witness.v"); 
        EC_POINT_mul(group, instance.C[i], witness.r[i], pp.h, witness.v[i], bn_ctx); 
    }
}

void test_bulletproof_boundary(size_t RANGE_LEN, size_t AGG_NUM, bool BOUNDARY_FLAG)
{
    Bullet_PP pp; 
...
{ 
    ...
    SplitLine_print(''); 
    AGG_NUM = 1; 
    test_bulletproof_boundary(RANGE_LEN, AGG_NUM, 0);  
    test_bulletproof_boundary(RANGE_LEN, AGG_NUM, 1);  
    global_finalize();
